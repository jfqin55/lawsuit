...
public class JettyFactory {
    ...
    /**
     * 创建Jetty Server, 以 DEFAULT_WEBAPP_PATH 为程序目录
     */
    public static Server createServerInSource(int port, String contextPath) {
    	
        Server server = new Server();
        
        //设置在 JVM 退出时关闭Jetty的钩子
        server.setStopAtShutdown(true);

        HttpConfiguration config = new HttpConfiguration();
        ServerConnector connector = new ServerConnector(server, new HttpConnectionFactory(config));
        connector.setPort(port);
        //解决 Windows 下，重复启动Jetty不报告端口冲突的问题
        connector.setReuseAddress(false);
        server.setConnectors(new Connector[]{connector});

        WebAppContext webContext = new WebAppContext(DEFAULT_WEBAPP_PATH, contextPath);
        //修改 webdefault.xml，解决 Windows 下 Jetty 锁住静态文件的问题
    ...
    /**
     * 快速重新启动程序，重载target/classes与target/test-classes
     */
    public static void reloadContext(Server server) throws Exception {
        ...
        System.out.println("Application reloaded successful");
    }
}
