...
@Controller
@RequestMapping(value = "/report/subject")
public class ReportSubjectController {
    
    private static final Logger logger = LoggerFactory.getLogger(ReportSubjectController.class);
    ...
    private static String MONTH_SQL_LEADER = "select subject_name as name, ifnull(sum(bill_amount - return_amount), 0) as data from t_b_bill where bill_status = ? and bill_type in (?, ?) and corp_id = ? and update_time >= str_to_date(?,'%Y-%m-%d') and update_time < date_add(str_to_date(?,'%Y-%m-%d'), interval 1 month) ";
    private static String MONTH_SQL_OTHERS = "select subject_name as name, ifnull(sum(bill_amount - return_amount), 0) as data from t_b_bill where bill_status = ? and bill_type in (?, ?) and bill_id in (select bill_id from t_b_bill_check where auditor_id = ? and check_status != '4') and update_time >= str_to_date(?,'%Y-%m-%d') and update_time < date_add(str_to_date(?,'%Y-%m-%d'), interval 1 month) ";
    
    @ResponseBody
	@RequestMapping("showMonthChart.do")
	public List<Map> showMonthChart(HttpServletRequest request, String month, String department_id) {
		try {
			...
			if(sessionUser.getDepartment_name().contains("领导班子") || Constant.UserRole.YS.equals(sessionUser.getRole_id())) {
			    ...
			    if(StringUtils.isBlank(department_id)) {
	                sql.append(" group by subject_name ").toString();
	            }else {
	                sql.append(" and department_id = ").append(department_id).append(" group by subject_name ").toString();
	            }
    ...
    private static String CUSTOM_SQL_LEADER = "select subject_name as name, ifnull(sum(bill_amount - return_amount), 0) as data from t_b_bill where bill_status = ? and bill_type in (?, ?) and corp_id = ? and update_time >= str_to_date(?,'%Y-%m-%d %H:%i:%s') and update_time <= str_to_date(?,'%Y-%m-%d %H:%i:%s') ";
    private static String CUSTOM_SQL_OTHERS = "select subject_name as name, ifnull(sum(bill_amount - return_amount), 0) as data from t_b_bill where bill_status = ? and bill_type in (?, ?) and bill_id in (select bill_id from t_b_bill_check where auditor_id = ? and check_status != '4') and update_time >= str_to_date(?,'%Y-%m-%d %H:%i:%s') and update_time <= str_to_date(?,'%Y-%m-%d %H:%i:%s') ";
    
    
    @ResponseBody
	@SuppressWarnings("all")
	@RequestMapping("showCustomChart.do")
	public List<Map> showCustomChart(HttpServletRequest request, String startDateStr, String endDateStr, String department_id) {
		try {
