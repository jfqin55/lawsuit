...
// 	            data: [['小微金融部',399.9], ['信贷管理部',291.5], ['办公室',186.4], ['电子银行部',159.2], ['信息科技部',134.0], ['计划财务部',116.0]] } ]
	...
	function searchByParams(e){
		...
		if(choice == '1'){
			...
			showMask();
			$.ajax({
				url: "${ctx}/report/dept/showMonthChart.do?month=" + month + "-01&subject_id=" + subject_id,
		...
		if(choice == '2'){
			...
			showMask();
			$.ajax({
				url: "${ctx}/report/dept/showCustomChart.do?startDateStr=" + startDateStr + "&endDateStr=" + endDateStr + "&subject_id=" + subject_id,
				success: function (data) {
					hideMask();
					mini.get("subButton").enable();
					var columnList = [];
	...
	function exportExcel(){
		mini.confirm("确认导出 excel 报表?", "提示", function(action){
            if (action == "ok") {
            	var url = '${ctx}/report/dept/exportExcel.do';
