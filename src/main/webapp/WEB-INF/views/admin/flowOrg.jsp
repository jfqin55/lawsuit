...
<body>    
    ...
    <div class="mini-fit">
        <ul id="tree" class="mini-tree" style="width:100%;height:100%;" 
            ...
            expandOnLoad="1" showCheckBox="true" checkRecursive="false" allowSelect="false" ondrawnode="onDrawNode">
        </ul>
    </div>                
...
//         if (isLeaf == false) e.showCheckBox = false; //父节点的CheckBox全部隐藏
    ...
    function SetData(data){
        ...
        if(record.action == "edit"){
        	var row = record.row;
        	$.post("${ctx}/admin/flow/getOrgTree.json", {check_flow_id: row.check_flow_id}, function(data) {
...
//         }
    ...
    function onOk(e){
//         var orgs = tree.getValue(true);//false-不包含父节点、true-包含
        var orgs = [];
        var orgNodes = tree.getCheckedNodes(true);//false-不包含父节点、true-包含
        $.each(orgNodes, function(i){
        	if(tree.isCheckedNode(orgNodes[i])) orgs.push(orgNodes[i].id);
        });
        var paramMap = {check_flow_id:record.row.check_flow_id, orgs:orgs.join()};
        $.post('${ctx}/admin/flow/bindOrgs.do', paramMap, function(data) {
            if (data == true) {
                CloseWindow("ok");
            } else {
